...
quiet=0 # Since we're using a GUI, don't need to worry about suppressing output
...
def do_run_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites):
...
def do_ind_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites):
    # Read in data as HydeData object
    data = hd.HydeData(infile, mapfile, outgroup, nind, ntaxa, nsites, quiet, ignore_amb_sites)
...
def do_boot_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites,reps):
    # Read in data as HydeData object
    data = hd.HydeData(infile, mapfile, outgroup, nind, ntaxa, nsites, quiet, ignore_amb_sites)
...
def check_input(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites,reps,analysis):
    all_good = True
    if analysis == 0:
...
def main(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites,reps,analysis):

    # Check the input
    good_to_go = check_input(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites,reps,analysis)
    if good_to_go:
        pass
    else:
        return 0
    ...
    if analysis == 1:
        do_run_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites)
    elif analysis == 2:
        do_ind_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites)
    elif analysis == 3:
        do_boot_hyde(infile,mapfile,outgroup,nind,ntaxa,nsites,outfile,tripletsfile,pvalue,ignore_amb_sites,reps)
    else:
...
if __name__ == "__main__":
    ...
    analysis = IntVar()
    ttk.Label(mainframe, text=long_description, wraplength=600, justify=LEFT).grid(column=1,row=1,columnspan=4, sticky=(W,E))
    ...
    ignore_box = ttk.Checkbutton(mainframe, text="Ignore missing/ambiguous sites?", variable=ignore_amb_sites)
    ignore_box.grid(column=2,row=11,sticky=(W,E))
    run_hyde = ttk.Radiobutton(mainframe, text='Full HyDe analysis', variable=analysis,value=1)
    run_hyde.grid(column=2,row=13, sticky=(W,E))
    ind_hyde = ttk.Radiobutton(mainframe, text='Individual-based HyDe analysis', variable=analysis,value=2)
    ind_hyde.grid(column=2,row=14,sticky=(W,E))
    boot_hyde = ttk.Radiobutton(mainframe, text='Individual-based bootstrapping analysis', variable=analysis,value=3)
    ...
    ttk.Separator(mainframe,orient=HORIZONTAL).grid(column=1,row=16, columnspan=4,sticky=(W,E))
    
    ttk.Button(mainframe,
        text="Run",
        command=lambda: main(infile.get(),
                             mapfile.get(),
                             outgroup.get(),
                             nind.get(),
                             ntaxa.get(),
                             ...
                             reps.get(),
                             analysis.get())).grid(column=4,row=17,sticky=W)
    
    ttk.Button(mainframe,
        text="Quit",
        command=lambda: sys.exit(0)).grid(column=1,row=17,sticky=W)
    ...
    ttk.Label(mainframe, text="Number of individuals:").grid(column=1,row=5,sticky=W)
    ttk.Label(mainframe, text="Number of taxa:").grid(column=1,row=6,sticky=W)
    ttk.Label(mainframe, text="Number of sites:").grid(column=1,row=7,sticky=W)
    ...
    ttk.Label(mainframe, text="Analysis options:").grid(column=1,row=12,sticky=W)
